module SPFP_pipeline_adder (clk, rst, A, B, out);
    ...
    reg [49:0] pipe_mantissa_sum;
    reg [7:0] pipe_sum_exp;
    reg pipe_is_zero;

    always @(posedge clk or posedge rst) begin
        if (rst) begin
            pipe_mantissa_sum <= 50'd0;
            pipe_sum_exp <= 8'd0;
            pipe_is_zero <= 1'b0;
        end else begin
            pipe_mantissa_sum <= mantissa_sum;
            pipe_sum_exp <= sum_exp;
            pipe_is_zero <= is_zero;
        end
    end
    ...
    always @(*) begin
        ...
        if (pipe_is_zero) begin
            final_sum = 32'd0; 
        end 
        else begin
            if (pipe_mantissa_sum[49] == 1'b1) begin
                final_sign   = 1'b1;                 
                abs_mantissa = ~pipe_mantissa_sum + 1'b1; 
            end else begin
                final_sign   = 1'b0;                  
                abs_mantissa = pipe_mantissa_sum;          
            end

            if (abs_mantissa[47] == 1'b1) begin
                final_exponent = pipe_sum_exp + 1'b1;
                final_mantissa = abs_mantissa[46:24];
            end 
            else if (abs_mantissa[46] == 1'b1) begin
                final_exponent = pipe_sum_exp;
                ...
                if (shift_count == 0 && abs_mantissa[46] == 1'b0) begin
                    ...
                    final_exponent = pipe_sum_exp - shift_count;
